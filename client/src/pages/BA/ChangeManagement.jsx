...
export default function ChangeManagement() {
  ...
  // Safely extract score once to prevent repeated undefined checks
  const currentRiskScore = selectedCR?.aiImpact?.riskScore || 15;
  const riskColorClass = currentRiskScore > 60 ? 'bg-red-500' : currentRiskScore > 30 ? 'bg-yellow-500' : 'bg-green-500';
  ...
  return (
    <div className="h-screen bg-[#F5F7FA] overflow-hidden flex flex-col">
      ...
      <div className="flex max-w-[1600px] w-full mx-auto pt-6 px-4 md:px-6 gap-8 pb-6 flex-1 min-h-0">
        ...
        <div className="hidden lg:block flex-shrink-0 h-full w-[260px]">
          <BASidebar />
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          ...
          <div className="flex flex-col lg:flex-row flex-1 min-h-0 bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className={`w-full lg:w-[35%] flex-col bg-[#FAFAFA] border-r border-gray-100 h-full flex-shrink-0 ${selectedCR ? 'hidden lg:flex' : 'flex'}`}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#FAFAFA] pr-2">
                {isLoading ? (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                            <div className={`w-2.5 h-2.5 rounded-full ${selectedCR?.id === cr.id ? 'bg-orange-500' : 'bg-gray-300'}`}></div>
                            <span className="text-[11px] font-bold text-orange-600">{cr.crId || "NEW-CR"}</span>
                        </div>
                ...
                <>
                  <div className="p-6 md:p-8 border-b border-gray-100 bg-white flex-shrink-0 z-10">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[12px] font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-md">{selectedCR.crId || "NEW-CR"}</span>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-[#F8FAFC] space-y-8 pr-2">
                    ...
                    <div>
                      ...
                      <div className="bg-white border border-blue-100 rounded-2xl p-6 shadow-sm relative overflow-hidden">
                         <div className={`absolute top-0 left-0 w-1 h-full ${riskColorClass}`}></div>
                         ...
                         <div className="flex justify-between items-end mb-2">
                            ...
                            <span className="text-[14px] font-bold text-navy">
                              {currentRiskScore}% Impact
                            </span>
                         </div>
                         
                         <div className="w-full bg-gray-100 rounded-full h-2.5 mb-6 overflow-hidden">
                            <div className={`h-full rounded-full transition-all duration-500 ${riskColorClass}`} style={{ width: `${currentRiskScore}%` }}></div>
                         </div>

                         <div className="bg-gray-50 p-5 rounded-xl border border-gray-100">
                           <p className="text-[13px] text-navy font-bold mb-1">Estimated Timeline Impact:</p>
                           <p className="text-[13px] text-gray-600 leading-relaxed">
                             {getTimelineRiskText(currentRiskScore)}
                           </p>
                         </div>
                              ...
                              <ul className="space-y-2">
                                {selectedCR.aiImpact.conflicts.map((conflict, i) => (
                                  <li key={i} className="flex items-start text-[12px] text-navy bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                                    ...
                                    <span><strong>{conflict?.taskId || 'Task'}:</strong> {conflict?.title || 'Unknown'} <span className="text-gray-400 ml-1">({conflict?.status || 'Pending'})</span></span>
                                  </li>
                                ))}
                              </ul>
                           </div>
