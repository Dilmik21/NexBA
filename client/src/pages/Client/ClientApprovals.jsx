...
import { Loader2, CheckCircle2, AlertTriangle, FileText, Search, Link as LinkIcon, ExternalLink, ChevronDown, X, Bug, Maximize, ArrowLeft } from "lucide-react";
...
export default function ClientApprovals() {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        setSelectedItem(prevSelected => {
          const incomingReqId = location.state?.selectedReqId || location.state?.id;
          
          // If we just navigated from the dashboard, auto-select the project
          if (incomingReqId && !isBackground) {
             const target = data.data.find(r => r.reqId === incomingReqId || r.id === incomingReqId);
             if (target) return target;
          }
          
          // Otherwise, stick to whatever is already selected
          if (prevSelected) {
        ...
        // SAFELY clear the routing state without triggering an infinite loop
        if (location.state?.selectedReqId || location.state?.id) {
  ...
  }, [currentUser, location.state]); // Removed navigate loop dependency
                ...
                <>
                  <div className="p-6 md:p-8 border-b border-gray-100 bg-white flex-shrink-0 z-10">
                    <div className="flex items-center gap-2 mb-3">
                      <button onClick={() => setSelectedItem(null)} className="lg:hidden p-1 mr-1 -ml-2 rounded-full hover:bg-gray-100 text-gray-500 transition-colors">
                         <ArrowLeft className="w-5 h-5" />
                      </button>
                      <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider ${getPriorityColor(selectedItem.priority)}`}>
      ...
      {isModalOpen && selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-6 overflow-y-auto max-h-[60vh] pr-2">
